...
- **Repository:** `<confirmed-owner>/<confirmed-repository>`
- **Configured from remote:** `<confirmed-remote-name>`
...
- Read an issue and comments with `gh issue view <number> --repo <owner/repository> --comments` and request structured JSON fields when the caller needs exact metadata.
- List issues with `gh issue list --repo <owner/repository> --state <state> --json ...` and use an explicit filter rather than assuming every open issue belongs to the Feature Run.
...
- Create an issue with `gh issue create --repo <owner/repository> --title <title> --body-file <file>`.
